...
function check_loggedin(){
	...
	$timestmp = time() - (1 * 60 * 60);
	$stmt = $dbh->prepare("select * from ci_sessions where id = '". mysql_escape_string( $session_id )."'  and timestamp > " . $timestmp);
	$stmt->execute()  ;
	 ...
	 if(count($rows) ==0)
		 return false;
	 $data = $rows[0]['data'];
	//$data = str_replace("__ci_last_regenerate", "",  $data);
	  $data = explode(';',$data);
	  
	  foreach($data as $dataItem){
		  $dataItemExp = explode('|',$dataItem);
		  if(!empty($dataItemExp[0])&& $dataItemExp[0] == 'identity')
			  return true;
	  }
...
$config = array(
	...
	*/
	...
	|
	*/
	'upload_dir' => dirname(dirname(dirname(dirname($_SERVER['PHP_SELF'])))) .'/uploads/',
	...
	*/
	...
	'fixed_path_from_filemanager'             => array( '../../../thumbs/' ), //fixed path of the image folder from the current position on upload folder
	...
	#                          4 / crop= resize and crop;
	 */
	'fixed_image_creation_option'             => array( 'landscape', 'auto' ), //set the type of the crop
	...
	'relative_image_creation_name_to_append'  => array( '.thumb'  ), //name to append on filename
