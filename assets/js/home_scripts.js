...
//		footerOffset = $('.mainfooter ').offset().top,
//		pageindex = $('.mainfooter ').closest('.main-content').find('.infos').length;
//	//if ($(window).width() > 768 && window_height > footerOffset && pageindex == 0)
//	//	$('.mainfooter').addClass('fixed');
//	//else
//	//	$('.mainfooter').removeClass('fixed'); section_page
//}
function changeHeight(){
	var headerHeight  = $('.header').outerHeight(),
		bannerHeight  = $('.page_banner').outerHeight(), 
		pageHeight	  = ($('.page').length > 0) ? $('.page').outerHeight() :(($('.section_page').length > 0) ? $('.section_page').outerHeight() : 0 ) ,
		footerHeight  = $('.mainfooter').outerHeight(),
		windowHeight  = $(window).height(),
		totalHeight   = (headerHeight + bannerHeight + pageHeight + footerHeight);
	if($(window).width() >= 768 && windowHeight > totalHeight && $('.videospage').length <= 0){
		$('.mainfooter').css('margin-top' , windowHeight -  totalHeight - Math.floor(footerHeight / 2) -  5 );
	}
}
function settop(){
	var offset = $('.q_connect , .socailbar').offset();
    var topPadding = 25;
	var headerHeight  = $('.header').outerHeight();
	var bannerHeight = ($('.slider_section').length > 0) ? $('.slider_section').outerHeight() : $('.page_banner').outerHeight();

	var hgt = Math.floor( headerHeight + bannerHeight  + topPadding);

	if($(window).width() >= 768){
		$('.q_connect , .socailbar').css('top',hgt);
	}else{
		$('.q_connect , .socailbar').removeAttr('style');
	}
...
$(document).ready(function () {
	...
	if ($('.file-upload-field').length > 0) {
		$("form").on("change", ".file-upload-field", function () {
			...
			$(this).parent(".file-upload-wrapper").removeClass("placeholder");
		});
	}
	//---------------------------------------------------------------
	...
	//---------------------------------------------------------------
	//if ($('#main_slider').length == 0 && $('.owl-carousel').length == 0) {
	//	setTimeout(function () {
	//		// fixheight();
	//	}, 100)
	//}
	...
	//var resizeHeight;
	//$(window).on('resize', function () {
	//	clearTimeout(resizeHeight);
	//	resizeHeight = setTimeout(fixheight, 500);
	//});
	//---------------------------------------------------------------
	...
	$(window).on("resize", function () {
		setTimeout(function () {
			if ($(window).width() > 700 && $('#sidebar').hasClass('open'))
	...
	$('#main_nav .has-submenu').click(function(event){
		event.preventDefault();
	});
	if ($('#partnerslist').length > 0) {
		var dir = $('#partnerslist').attr('dir');
	...
	if ($('.list-locations').length > 0) {
		$('.list-locations a').click(function (event) {
			event.preventDefault();
			var lat, long;
		...
		$('.list-locations select').change(function (event) {
			var lat, long;
			lat = parseFloat($(this).children(":selected").data('lat'));
			long = parseFloat($(this).children(":selected").data('long'));
			newLocation(lat, long);
		});
	}
	//---------------------------------------------------------------
...
function initialize(latt, long) {
	...
	var image = {
		url: base_url + 'assets/img/marker.png'
	};
	var marker, i;
